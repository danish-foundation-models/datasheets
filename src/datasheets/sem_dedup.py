...
def parse_arguments() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--dataset-path",
        type=Path,
        help="Optional path of the dataset to deduplicate",
    )
...
def main():
    ...
    try:
        ...
        if not args.dataset_path:
            logger.info("No dataset path provided, looking for latest version")
            dataset_path = get_dataset_path(args.dataset_name, logger)
        else:
            logger.info(f"Using provided dataset path: {args.dataset_path}")
            if not args.dataset_path.exists():
                raise FileNotFoundError(
                    f"Dataset path does not exist: {args.dataset_path}"
                )
