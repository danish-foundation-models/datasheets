...
DOMAINS_CHOICES = [
    "Books", "Conversation", "Dialect", "Encyclopedic", "Legal", "News", 
    "Other", "Readaloud", "Social Media", "Speeches", "Spoken", "Subtitles", "Web"
]
...
def get_user_input():
    ...
    data['license_id'] = questionary.select(
        "Select the license ID:",
        choices=LICENSE_ID_CHOICES
    ).ask()
    ...
    data['short_description'] = questionary.text(
        "Enter the short description (for the top of the card):",
        multiline=True
    ).ask()
    data['long_description'] = questionary.text(
        "Enter the long description (main body):",
        multiline=True
    ).ask()
    data['license_information'] = questionary.text(
        "Enter the full license information:",
        multiline=True
    ).ask()
    ...
    if questionary.confirm("Add a contribution notice from a company?").ask():
        ...
        data['contribution_line'] = f"This data has been contributed by [{company_name}] through the [Dansk Sprogmodel Konsortium](https://alexandra.dk/dsk)."
    else:
...
def create_card_content(template_content, data):
    ...
    body_content = body_content.replace("LICENSE_INFORMATION", data['license_information'])
    body_content = body_content.replace("CITATION_INFORMATION", data['citation_information'])
    ...
    body_content = body_content.replace("<!-- CONTRIBUTION-PLACEHOLDER -->", data['contribution_line'])
