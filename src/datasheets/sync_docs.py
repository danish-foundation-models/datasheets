...
def download_repo(
    ...
    if site_path.exists():
        logger.info(
            "Found dfm site locally. Ensuring main branch and pulling latest changes..."
        )
...
def update_mkdocs(site_path: Path):
    ...
    datasheet_nav = "\n".join(
        [f"        - {name}: {path}" for name, path in datasheet_entries]
    )
...
def main():
    ...
    sheet.body = sheet.replace_tag(
        package=generate_html_table(domain_table), tag="DOMAIN TABLE"
    )

    logger.info("Creating license table")
    license_table = create_grouped_table_str(group="License")
    sheet.body = sheet.replace_tag(
        package=generate_html_table(license_table), tag="LICENSE TABLE"
    )
